...
void Maze::generateDoor()
{
	...
	int doorRange = int(path.size() / 4.f);
	...
	std::pair<unsigned int, unsigned int> xyPos = this->dj.getMapIdx(unsigned int(path[idx].getTo()->getIndex()), unsigned int(this->grid[0].size()));
