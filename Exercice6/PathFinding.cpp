...
void ai::Graph::calculateCost(std::vector<std::vector<char>>& map, unsigned int y, unsigned int x, int& cost)
{
	if (map[y][x] == '#')
		cost = 99;
	else if (map[y][x] == 'E')
...
//Graph
ai::Graph::Graph(std::vector<std::vector<char>>& map)
{
	int row = int(map.size());
	int column = int(map[0].size());
	...
	for (int y = 0; y < row; y++)
		for (int x = 0; x < column; x++)
		{
			...
			if (y > 0)
			{
				this->calculateCost(map, y - 1, x, cost);
				if (cost != 99)
				{
			...
			if (x > 0)
			{
				this->calculateCost(map, y, x - 1, cost);
				if (cost != 99)
				{
...
std::pair<unsigned int, unsigned int> ai::Dijkstra::getMapIdx(unsigned int pathIndx, unsigned int row)
{
	return std::pair<unsigned int, unsigned int>(pathIndx / row, pathIndx % row);
}