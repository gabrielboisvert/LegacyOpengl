...
namespace ai
{
	...
	public:
		...
		void calculateCost(std::vector<std::vector<char>>& map, unsigned int y, unsigned int x, int& cost);
		Graph(std::vector<std::vector<char>>& map);
		Node& addNode(unsigned int idx);
		Node* getNode(unsigned int idx);
		std::list<Node>& getNodes() { return this->nodes; };
	...
	public:
		...
		std::pair<unsigned int, unsigned int> getMapIdx(unsigned int pathIndx, unsigned int row);
	};
}